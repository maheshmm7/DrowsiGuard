...
drowsy_folder = os.path.join(path, 'drowsy_images')
sleepy_folder = os.path.join(path, 'sleepy_images')
os.makedirs(drowsy_folder, exist_ok=True)
os.makedirs(sleepy_folder, exist_ok=True)

# Counters to limit the number of saved images
drowsy_image_count = 0
sleepy_image_count = 0
MAX_IMAGES = 5
...
def detect_gaze_direction(landmarks, width, height):
    """Detect the direction of the gaze."""
...
def draw_iris(frame, landmarks, indexes, color):
    """Draw circles around the iris landmarks."""
    for i in indexes:
...
def save_frame(frame, folder, state):
    """Save the current frame to an image file."""
    timestamp = time.strftime('%Y%m%d_%H%M%S')
    filename = os.path.join(folder, f'{state}_{timestamp}.jpg')
...
def get_head_pose(shape):
    """Estimate head pose using predefined model points and facial landmarks."""
...
def is_drowsy(eye_state_history):
    """Determine if the user is drowsy based on eye state history."""
...
# Constants for drowsiness detection thresholds
...
while True:
    try:
        ...
        for (x, y, w, h) in right_eye:
            ...
            print(f"Right Eye coordinates: x={x}, y={y}, w={w}, h={h}")
            count = count + 1
        ...
        for (x, y, w, h) in left_eye:
            ...
            print(f"Left Eye coordinates: x={x}, y={y}, w={w}, h={h}")
            count = count + 1
        ...
        if len(eye_state_history) >= eye_state_history.maxlen and is_drowsy(eye_state_history):
            ...
            if elapsed_time > PROLONGED_DROWSINESS_THRESHOLD:
                ...
                if sleepy_image_count < MAX_IMAGES:
                    save_frame(frame, sleepy_folder, 'Sleepy')
                    sleepy_image_count += 1
                score += 2  # Increment score by 2 for prolonged drowsiness
                ...
                if drowsy_image_count < MAX_IMAGES:
                    save_frame(frame, drowsy_folder, 'Drowsy')
                    drowsy_image_count += 1
                score += 1  # Increment score by 1 for short drowsiness
            ...
            score = max(score - 1, 0)  # Decrement score but not below 0
